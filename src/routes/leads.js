...
router.post("/leads", (req, res) => {
  ...
  ).run(L.name, L.phone, L.city, L.budget, JSON.stringify(L.preferences), L.source);
  res.status(201).json({ ok: true, id: r.lastInsertRowid });
});

router.post("/viewings", (req, res) => {
  const db = req.app.locals.db;
  const v = validateViewing(req.body, db);
  if (!v.ok) {
  ...
  ).run(V.listing_id, V.name, V.phone, V.preferred_date);
  res.status(201).json({ ok: true, id: r.lastInsertRowid });
});
